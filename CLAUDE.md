...
assumes but 0014 never actually built. Fee definition, invoice generation, payment recording, payment reversal, and
now collection sessions all have real use cases and API routes — §13's
schema is fully wired up. `POST /payments` is live, with the gapless
...
— see PR #46 for why). `POST /payments/:id:reverse` writes a NEW payment row
rather than deleting the original (invariant 1) — the original receipt
number stays permanently consumed. `POST /collection-sessions` (+
...
| **Every endpoint, generated from the Zod schemas** | [`docs/API.md`](docs/API.md) · [`openapi.json`](docs/openapi.json) |
